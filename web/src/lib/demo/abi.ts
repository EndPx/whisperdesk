// abi.ts — ported verbatim from scripts/e2e/lib/abi.mjs: minimal ethers.js ABI fragments, kept
// hand-written and small (rather than importing forge's full `out/DvPEscrow.sol/DvPEscrow.json`
// artifact) so this app has no build-step dependency on `forge build` having just run. Transcribed
// from contracts/src/DvPEscrow.sol — keep in sync if that contract's public interface changes.
