// POST /api/taker/rfq/confirm — read back the judge's own submitRfq receipt and publish the RFQ.
//
// Two things happen here that cannot happen client-side. The enclave acknowledges an on-chain
// instruction asynchronously, so the ack has to be polled from the proxy; and the resulting rfqId
// has to land in the shared queue for makers in other browsers to find.
//
// The taker stamped in SealedRfqSubmitted is checked against the address claiming to have sent it.
// That is the difference between "this RFQ is yours" and "you told us it was".
